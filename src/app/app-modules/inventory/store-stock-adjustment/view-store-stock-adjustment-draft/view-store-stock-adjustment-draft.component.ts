...
import { lucideEye, lucidePencil, lucideSearch } from '@ng-icons/lucide';
...
@Component({
  ...
  viewProviders: [provideIcons({ lucideEye, lucidePencil, lucideSearch })],
  imports: [
    FormsModule,
    NgIf,
    NgFor,
...
export class ViewStoreStockAdjustmentDraftComponent implements OnInit, DoCheck {
  ...
  filterStockAdjustmentList(filterTerm: any) {
    if (!filterTerm) {
      ...
      this.filteredStockAdjustmentList = new TableDataSource<any>(
        this.filteredStockAdjustmentList.data,
      );
      this.filteredStockAdjustmentList.paginator = this.paginator;
    } else {
